...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = ( tail union that ) incl head
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet =
    if ( !p(elem) ) left.filter0( p, right.filter0( p, accu ) )
    else left.filter0( p, right.filter0( p, accu.incl( elem ) ) )
    
  def ascendingByRetweet: Trending =  new NonEmptyTrending( findMin, tail.ascendingByRetweet )
